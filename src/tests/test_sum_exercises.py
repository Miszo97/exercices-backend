...
def test_sum_exercises():
    ...
    exercises = [
        ...
        ExerciseEntry(
            name="Running", reps=None, duration=30, unit="minutes", date=today
        ),
        ExerciseEntry(
            name="Running", reps=None, duration=20, unit="minutes", date=today
        ),
        ExerciseEntry(
            name="Cycling", reps=None, duration=45, unit="minutes", date=today
        ),
    ]
    summed = sum_exercises(exercises)
    assert len(summed) == 3
    assert summed == [
        ExerciseDay(name="Push-up", reps=25, duration=0, unit=None, date=today.date()),
        ExerciseDay(
